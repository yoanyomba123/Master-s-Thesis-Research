function energyField = itersegmentationRSF(imgInput, Parameter)
...
u = initialLSF;
if (figureBose == 1)
	figure;maxscreen();imagesc(Img, [0, 255]);colormap(gray);hold on;axis off,axis equal
...
mu = Parameter.uReal;% coefficient of the level set (distance) regularization term P(\phi)
...
if (figureBose == 1)
    ...
    % mesh the level set function if needed
	% figure;
	% mesh(u);
