...
% along B-Scans. 
% 
% [LINES, FZ] = findRetinaExtrema(NORMOCTIMG, PARAMS, C, MODE, LINEREGION)
% Helper function to find contrast changes in A-Scans all
...
% then detect the extrema on the gradient (i.e. edges on the original image)
% 
...
%   the gradient in Z-direction previous to extrema finding. 
%   (suggestion: 3);
...
%   allregion: All C(1) extremas (min/max treated the same, e.g. no 
%              differentiation between rising/falling edges) are detected 
%              within the requested region
%   anything else: The mode is passed to the extremafinder function
% LINEREGION: Only edges within this region are detected. The lineregion is 
%   defined by two lines according to OCTSEG standard, without holes.
% LINES: The detected contrast changes.
% FZ: Gradient in z-direction of the A-Scans.
%
...
if nargin < 5
    lineregion = [zeros(1,size(normOctimg,2), 'single') + 2; ...
        zeros(1,size(normOctimg,2), 'single') + size(normOctimg,1)];
end
